...
def extract_ss_arguments(ptree, conn, arg1, arg2):
    labels = []
    ...
    for i, _ in enumerate(sent_features):
        if i in arg1:
            ...
            labels.append("NULL")
    return sent_features, labels


def extract_ps_arguments(ptree, conn, arg2):
    labels = []
    sent_features = get_features(ptree, conn)
    for i, _ in enumerate(sent_features):
        if i in arg2:
...
def generate_pdtb_features(pdtb, parses):
    ...
    for relation in filter(lambda i: i['Type'] == 'Explicit', pdtb):
        ...
        if arg1_sentence_id == arg2_sentence_id:
            ss_features.append(extract_ss_arguments(ptree, indices, arg1, arg2))
        # Arg1 is in the previous sentence (PS)
        elif (arg2_sentence_id - arg1_sentence_id) == 1:
            ps_features.append(extract_ps_arguments(ptree, indices, arg2))
...
class ArgumentExtractCRF:
    ...
    def extract_arguments(self, ptree, relation):
        ...
        if relation['ArgPos'] == 'SS':
            ...
            arg1_prob = probs_max[arg1].mean()
            arg2_prob = probs_max[arg2].mean()
