...
class DiscourseParser(object):
    ...
    def save(self):
        self.connClassifier.save('tmp/')
        self.argPosClassifier.save('tmp/')
        self.argExtractClassifier.save('tmp/')
        self.explicitClassifier.save('tmp/')

    def load(self):
        self.connClassifier.load('tmp/')
        self.argPosClassifier.load('tmp/')
        self.argExtractClassifier.load('tmp/')
        self.explicitClassifier.load('tmp/')
...
if __name__ == '__main__':
    ...
    relations = parser.parse_file('../discourse/data/conll2016/en.dev/parses.json')
    with open('tmp/output_dev.json', 'w') as fh:
        fh.write('[\n')
